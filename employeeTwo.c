...
static int compareEmployeeSalary(const void *targetPtr, PtrToConstEmployee tableValuePtr)
{
    ...
    return (targetSalary != tableValuePtr->salary);
}
...
PtrToEmployee searchEmployeeByPhone(PtrToConstEmployee ptr, int size, char targetNumber[])
{
    return searchEmployeeTable(ptr, size, targetNumber, compareEmployeePhone);
}
